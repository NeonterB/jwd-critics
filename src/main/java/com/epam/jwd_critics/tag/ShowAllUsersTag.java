                ...
                for (int i = 0, j = 0; i < users.size() && i < USERS_PER_PAGE; i++) {
                    ...
                    if (!user.getId().equals(userDTO.getId())) {
                        if (userDTO.getStatus().equals(Status.BANNED)) {
                            writer.write("<a href=\"" + contextPath + "/controller?command=unban_user&userId=" + userDTO.getId() + "\">Unban</a>");
                        } else {
                            writer.write("<a href=\"" + contextPath + "/controller?command=ban_user&userId=" + userDTO.getId() + "\">Ban</a>");
                        }
                    }
                    writer.write("</div>");
